...
void stemmerPorter::strip_prefixes(char *string) {
  char *prefixes[] = {(char *)"kilo",   (char *)"micro",
                      (char *)"milli",  (char *)"intra",
                      (char *)"ultra",  (char *)"mega",
                      (char *)"nano",   (char *)"pico",
                      (char *)"pseudo", 0};
  int i;
...
void stemmerPorter::step_1(char *string) {
  if (strlen(string) >= GtamStem) {
    ...
    stem = new char[GtamStem];
    cerr << "ERROR SOLUCIONADO: palabra: " << string
         << " a stemmerizar de longitud: " << strlen(string)
         << " mayor que el máximo previsto: " << TAMANYO_MAX_PAL << endl;
  }

  //	if(strlen(string) >= TAMANYO_MAX_PAL) {
  //		cerr << "ERROR: palabra: " << string << " a stemmerizar de
  //longitud: " << strlen(string) << " mayor que el máximo previsto: " <<
  //TAMANYO_MAX_PAL << endl; 		exit(EXIT_STEMMER);
  //	}
    ...
    if (((has_suffix(string, (char *)"ed", stem) == TRUE) ||
      ...
      if ((has_suffix(string, (char *)"at", stem) == TRUE) ||
        ...
        if ((string[length - 1] == string[length - 2]) &&
            (string[length - 1] != 'l') && (string[length - 1] != 's') &&
            (string[length - 1] != 'z'))
          string[length - 1] = '\0';
  ...
  if (has_suffix(string, (char *)"th", stem) ==
      TRUE) // ING añadido para que quite 50th y deje 50
    string[strlen(string) - 2] = '\0';
}
...
void stemmerPorter::step_4(char *string) {
  char *suffixes[] = {(char *)"al",    (char *)"ance",
                      (char *)"ence",  (char *)"er",
                      (char *)"ic",    (char *)"able",
                      (char *)"ible",  (char *)"ant",
                      (char *)"ement", (char *)"ment",
                      (char *)"ent",   (char *)"sion",
                      (char *)"tion",  (char *)"ou",
                      (char *)"ism",   (char *)"ate",
                      (char *)"iti",   (char *)"ous",
...
int stemmerPorter::has_suffix(char *word, char *suffix, char *stem) {
  ...
  //		cerr << "ERROR: palabra: " << word << " a stemmerizar de
  //longitud: " << strlen(word) << " mayor que el máximo previsto: " <<
  //TAMANYO_MAX_PAL << endl; 		exit(EXIT_STEMMER);
  //	}
...
void stemmerPorter::stemmer(string &cad, const int &numero) {
  ...
  case 2: // ingleis
    ...
    //           cout << "STEMMER " << numero << ": pal: " << cad << " stem: "
    //           << cadena << endl;
    cad = cadena;
    break;
  default:
